...
namespace sdds_ws9
{
	...
	ProcessData::ProcessData(std::string filename) {
		...
		while (input)
		{
			input.read(reinterpret_cast<char*>(&total_items), sizeof(total_items));
			data = new int[total_items];
			input.read(reinterpret_cast<char*>(&data), sizeof(int)*total_items);
		}
		input.close();
	...
	int ProcessData::operator()(std::string filename, double& avg, double& variance)
	{
		...
		f.write(reinterpret_cast<char*>(&data), sizeof(int)* total_items);
		f.close();
